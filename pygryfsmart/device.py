...
class Device(RS232Handler):
    ...
    async def set_rol(self , id: int , pin: int , time: int , operation: SCHUTTER_STATES | int):
    ...
    async def reset(self , module_id: int , update_states: bool):
        if module_id == 0:
            ...
            if update_states == True:
                module_count = len(self.feedback.data["OUT"])
                await asyncio.sleep(2)
                states = self.feedback.data["OUT"]
                for i in range(module_count):
                    tabble = list(self.feedback.data["OUT"][i+1].values())
    ...
    async def __states_update_interval(self, time: int):
        try:
            while True:
                module_count = max(len(self.feedback.data["IN"]), len(self.feedback.data["OUT"]))
